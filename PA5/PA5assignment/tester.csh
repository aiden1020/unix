...
switch ( $1 )
   case "[Rr]*":
      ...
      exit
   case "[Ff]*":
      @ suit = `seq 1 13 40 | sort -R | head -n 1` #Use seq, sort & head to randomly choose from 1, 14, 27 & 40
      ...
      exit
   case "[Ss]*":
      @ f = `seq 1 8 | sort -R | head -n 1` # Use seq, sort & head to create a random number from 1 and 8
      set h
      foreach i ( `seq 0 4`)
         @ suit = `seq 1 13 40 | sort -R | head -n 1` # This is the same as the earlier "@ suit =" line.
         set h=( $h `expr $i + $f + $suit ` ) # Add next card to the straight, using above suit.
      end
      echo $h | tr " " "\n" | sort -R> nums
      breaksw
   case "4*":
...
sed -n `head -5 nums| sed 's/\([0-9]*\)/\1p;/g' | tr -d '\n'` allcards
